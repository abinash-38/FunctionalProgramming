...
public class FP04CustomClass {
	...
	public static void main(String[] args) {
		...
		Predicate<Course1> reviewScoreGreaterThan95Predicate 
			= course -> course.getReviewScore() > 95;

		Predicate<Course1> reviewScoreGreaterThan90Predicate 
			= course -> course.getReviewScore() > 90;
			
		Predicate<Course1> reviewScoreLessThan90Predicate 
			= course -> course.getReviewScore() < 90;
		...
		Comparator<Course1> comparingByNoOfStudentsIncreasing 
					= Comparator.comparingInt(Course1::getNoOfStudents);
		...
		Comparator<Course1> comparingByNoOfStudentsDecreasing 
					= Comparator.comparingInt(Course1::getNoOfStudents).reversed();
		...
		Comparator<Course1> comparingByNoOfStudentsAndNoOfReviews 
				= Comparator.comparingInt(Course1::getNoOfStudents)
							.thenComparingInt(Course1::getReviewScore)
							.reversed();
		...
		System.out.println(
				...
				.orElse(new Course1("Kubernetes", "Cloud", 91, 20000))
				);
		...
		System.out.println(
				courses.stream()
				.filter(reviewScoreGreaterThan95Predicate)
				.mapToInt(Course1::getNoOfStudents)
				.sum());//88000
		...
		System.out.println(
				courses.stream()
				.collect(Collectors.groupingBy(Course1::getCategory)));
		...
		System.out.println(
				courses.stream()
				.collect(Collectors.groupingBy(Course1::getCategory, Collectors.counting())));
		//{Cloud=4, FullStack=1, Microservices=2, Framework=2}

		System.out.println(
				courses.stream()
				.collect(Collectors.groupingBy(Course1::getCategory, 
						Collectors.maxBy(Comparator.comparing(Course1::getReviewScore)))));
		...
		System.out.println(
				courses.stream()
				.collect(Collectors.groupingBy(Course1::getCategory, 
						Collectors.mapping(Course1::getName, Collectors.toList()))));
		//{Cloud=[AWS, Azure, Docker, Kubernetes], FullStack=[FullStack], Microservices=[API, Microservices], Framework=[Spring, Spring Boot]}
		
		
		Predicate<Course1> reviewScoreGreaterThan95Predicate2 
						= createPredicateWithCutoffReviewScore(95);

		Predicate<Course1> reviewScoreGreaterThan90Predicate2 
						= createPredicateWithCutoffReviewScore(90);

	}

	private static Predicate<Course1> createPredicateWithCutoffReviewScore(int cutoffReviewScore) {
